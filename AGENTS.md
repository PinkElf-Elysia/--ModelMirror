...
- 修改知识流水线必须运行 `test_rag_pipeline.py`、`test_rag_pipeline_execute.py`、`test_rag_retrieval_v2.py`、RAG integration 和 workflow citation 回归，并重建 Docker 做双索引、混合检索、Rerank 与版本切换验收。

## 12. 持久化与工作区隔离

- `server/xperts/storage/`、`server/xpert_runtime/storage/`、`server/rag/storage/` 与上传目录是运行数据，不得提交。
