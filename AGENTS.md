...
- Knowledge Pipeline Job 必须固定 draft version 和源快照，按 `load/process/chunk/embed/store` 顺序更新持久化 stage 状态。
...
- Graph 节点预览不得写 Draft、Job、索引或版本，最多返回 20 条截断摘要。Embedding、双索引和检索预览只能返回安全能力/profile；图像理解在真实实现前必须保持 disabled。
...
- 修改知识流水线必须运行 `test_rag_pipeline_graph.py`、`test_rag_processor.py`、`test_rag_pipeline.py`、`test_rag_pipeline_execute.py`、`test_rag_retrieval_v2.py`、RAG integration 和 workflow citation 回归，并重建 Docker 做 Graph revision、节点预览、Processor、双索引、混合检索、Rerank 与版本切换验收。
