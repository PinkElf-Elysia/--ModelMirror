...
该 Processor 阶段之后的 `XPERT-KNOWLEDGE-CANVAS-01` 已由本文顶部增量完成：数据源、处理器、分块器、Embedding 与索引配置现在会编译为现有 Knowledge Pipeline Job，没有新建第二套执行器。OCR/VLM、检索评估、知识读写 Agent 按既定顺序后续推进；GraphRAG 继续暂缓。
