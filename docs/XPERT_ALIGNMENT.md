...
下一步固定为 `XPERT-KNOWLEDGE-AGENT-01`，提供知识搜索、引用和审批写入 Toolset。图片向量、多模态 Embedding、版面坐标和 GraphRAG 继续暂缓。
...
离线评估与 Promotion Gate 已由顶部增量完成；下一步推进知识读写 Agent。图片向量、多模态 Embedding、版面坐标和 GraphRAG 继续暂缓。
...
该画布基线已由多模态与评估增量扩展为可选视觉阶段和版本 Promotion Gate；后续推进知识读写 Agent。GraphRAG 继续暂缓。
...
该 Processor 阶段之后的 Knowledge Canvas、OCR/VLM 与离线检索评估均已完成：数据源、处理器、分块器、Embedding 与索引配置编译为现有 Knowledge Pipeline Job，候选版本可通过 Evaluation Run 与 Promotion Gate 验收。知识读写 Agent 是下一步；GraphRAG 继续暂缓。
...
| Knowledge Pipeline | 部分实现 | FileAsset/Artifact/Chunk/CitationAnchor、结构感知 Processor、General/QA/Summary、可执行 Graph、逐文档/逐视觉页恢复、图片与扫描 PDF 的 OCR/VLM、版本化 ingestion job、递归与父子分块、向量/FTS5 双索引、混合检索、Rerank、离线评估、Promotion Gate、预览、激活/回滚 | 本地单进程 worker；旧上传路径保留 vector-only legacy index；评估标签需人工维护；图片向量、版面坐标与 GraphRAG 尚未接入 | `XPERT-KNOWLEDGE-AGENT-01` |
