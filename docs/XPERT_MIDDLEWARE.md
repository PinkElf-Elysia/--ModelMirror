...
下一阶段进入 `XPERT-MIDDLEWARE-CONSOLIDATION-07`。客户端当前标签页桥、服务端 Browser sidecar、无网 Sandbox 与 Automation Coordinator 保持独立安全边界。
...
当前提供上下文压缩、结构化输出、持久 Todo、LLM 工具选择、可恢复 HITL、Sandbox/Skill、隔离 Browser、Client Tools、Scheduler、Ralph Loop、Knowledge Writer 和 Plugin Hooks。Xpert 源码仅用于领域模型与交互参考，当前实现为 React/FastAPI 原生重写。
...
1. `XPERT-MIDDLEWARE-CONSOLIDATION-07`：基于真实使用补齐缺失策略、统一测试矩阵、发布预检和配置迁移。

Automation 已进入当前范围；分布式调度、Workflow Engine V2 和公开 App 自动化仍不属于当前范围。
