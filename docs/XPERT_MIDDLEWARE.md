...
后续阶段依次为 `XPERT-MIDDLEWARE-AUTOMATION-06` 和 `XPERT-MIDDLEWARE-CONSOLIDATION-07`。客户端当前标签页桥与服务端 Browser sidecar 保持独立安全边界。
...
当前提供上下文压缩、结构化输出、持久 Todo、LLM 工具选择、可恢复 HITL、Sandbox/Skill、隔离 Browser 和 Client Tools。Xpert 源码仅用于领域模型与交互参考，当前实现为 React/FastAPI 原生重写。
...
1. `XPERT-MIDDLEWARE-AUTOMATION-06`：定时任务、Ralph Loop、知识写入器和插件 Hook。
2. `XPERT-MIDDLEWARE-CONSOLIDATION-07`：补齐剩余能力并统一发布预检和测试矩阵。

自动化和 Workflow Engine V2 仍不属于当前范围。
